...
interface FloorplannerCameraControlsProps {
  blueprint3d: any;
}

const FloorplannerCameraControls: React.FC<FloorplannerCameraControlsProps> = ({ blueprint3d }) => {
  ...
  const handleZoomClick = (level: number) => {
    ...
    for (let i = 0; i < steps; i++) {
      if (diff > 0) {
        // Zoom in
        const newCmPerPixel = blueprint3d.floorplanner.cmPerPixel / zoomFactor;
        blueprint3d.floorplanner.cmPerPixel = newCmPerPixel;
        blueprint3d.floorplanner.pixelsPerCm = 1.0 / newCmPerPixel;
      } else {
        // Zoom out
        const newCmPerPixel = blueprint3d.floorplanner.cmPerPixel * zoomFactor;
        blueprint3d.floorplanner.cmPerPixel = newCmPerPixel;
    ...
    blueprint3d.floorplanner.wallWidth = 10.0 * blueprint3d.floorplanner.pixelsPerCm;
  ...
  const pan = (direction: 'UP' | 'DOWN' | 'LEFT' | 'RIGHT') => {
    ...
    switch (direction) {
      case 'UP':
        floorplanner.originY -= panSpeed;
        break;
      case 'DOWN':
        floorplanner.originY += panSpeed;
        break;
      case 'LEFT':
        floorplanner.originX -= panSpeed;
        break;
      case 'RIGHT':
        floorplanner.originX += panSpeed;
        break;
    ...
    <>
      ...
      {showNavigate && (
        <div className="navigation-circle">
          ...
          <button className="nav-center-btn" onClick={resetView} title="Reset View">
      ...
      {!showNavigate && (
        <>
          ...
          <button className="navigate-btn" onClick={() => setShowNavigate(true)} title="Navigate">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
export default FloorplannerCameraControls;
