    ...
    (function (Items) {
        ...
        var Item = (function (_super) {
            ...
            function Item(model, metadata, geometry, material, position, rotation, scale) {
                ...
                /** */
                this.getObstructFloorMoves = function () {
            ...
            FloorItem.prototype.isValidPosition = function (vec3) {
                ...
                if (this.obstructFloorMoves) {
                    ...
                    for (var i = 0; i < objects.length; i++) {
                        ...
                        if (!BP3D.Core.Utils.polygonOutsidePolygon(corners, otherCorners, vec3.x, vec3.z) ||
                            BP3D.Core.Utils.polygonPolygonIntersect(corners, otherCorners)) {
...
(function (BP3D) {
    ...
    (function (Floorplanner) {
        var DimensionRenderer = (function () {
            function DimensionRenderer(context, viewmodel, style) {
             ...
             */
            DimensionRenderer.prototype.drawEdgeDimension = function (edge) {
                ...
                var normal = this.calculateOutwardNormal(edge);
                var wallDir = this.calculateWallDirection(start, end);
                ...
                var dimensionText = BP3D.Core.Dimensioning.cmToMeasure(length);
             ...
             */
            DimensionRenderer.prototype.calculateOutwardNormal = function (edge) {
                var start = edge.interiorStart();
                var end = edge.interiorEnd();
                ...
                var centerX = 0;
                var centerY = 0;
                roomCorners.forEach(function (corner) {
                    centerX += corner.x;
                    centerY += corner.y;
                });
                ...
                var toCenterX = midX - centerX;
                var toCenterY = midY - centerY;
                // Check if normal points away from center
                var dotProduct = normalX * toCenterX + normalY * toCenterY;
                if (dotProduct < 0) {
             ...
             */
            DimensionRenderer.prototype.calculateWallDirection = function (start, end) {
                var dx = end.x - start.x;
                var dy = end.y - start.y;
                var length = Math.sqrt(dx * dx + dy * dy);
             ...
             */
            DimensionRenderer.prototype.drawExtensionLines = function (start, end, normal) {
                var extOffset = this.style.offset + this.style.extensionOverhang;
                var extStart = {
             ...
             */
            DimensionRenderer.prototype.drawLine = function (x1, y1, x2, y2) {
                var screenX1 = this.viewmodel.convertX(x1);
                var screenY1 = this.viewmodel.convertY(y1);
                var screenX2 = this.viewmodel.convertX(x2);
                var screenY2 = this.viewmodel.convertY(y2);
             ...
             */
            DimensionRenderer.prototype.drawTick = function (x, y, dirX, dirY) {
                // Perpendicular to wall direction
                var perpX = -dirY;
                var perpY = dirX;
                var x1 = x - perpX * this.style.tickLength;
                var y1 = y - perpY * this.style.tickLength;
                var x2 = x + perpX * this.style.tickLength;
                var y2 = y + perpY * this.style.tickLength;
                this.context.strokeStyle = this.style.lineColor;
                this.context.lineWidth = this.style.lineWidth;
             ...
             */
            DimensionRenderer.prototype.drawText = function (text, x, y) {
                var screenX = this.viewmodel.convertX(x);
                var screenY = this.viewmodel.convertY(y);
                ...
                var padding = 4;
                var bgWidth = metrics.width + padding * 2;
                var bgHeight = 16;
                // Draw background
                this.context.fillStyle = this.style.backgroundColor;
                this.context.fillRect(screenX - bgWidth / 2, screenY - bgHeight / 2, bgWidth, bgHeight);
                // Draw text
             ...
             */
            DimensionRenderer.prototype.updateStyle = function (style) {
                for (var key in style) {
                    if (style.hasOwnProperty(key)) {
                        this.style[key] = style[key];
                    }
             ...
             * Get current style
             */
            DimensionRenderer.prototype.getStyle = function () {
                var styleCopy = {
        ...
        Floorplanner.DimensionRenderer = DimensionRenderer;
...
(function (BP3D) {
    ...
    (function (Floorplanner) {
        ...
        // room config
        var roomColor = "#e8dcc4"; // Beige/tan color like architectural drawings
        var roomLabelColor = "#666666";
        var roomLabelBgColor = "rgba(255, 255, 255, 0.9)";
        // wall config
        var wallWidth = 8;
        var wallWidthHover = 8; // Same as wallWidth for consistent edge display
        var wallColor = "#5a5a5a"; // Darker gray for walls
        var wallColorHover = "#008cba";
        var edgeColor = "#2a2a2a"; // Dark gray for wall edges
        ...
        // item config
        var itemColor = "#b0b0b0"; // Light gray for furniture
        var itemBorderColor = "#404040"; // Dark gray border
        ...
        var FloorplannerView = (function () {
            /** */
            function FloorplannerView(floorplan, viewmodel, canvas, model) {
                this.floorplan = floorplan;
                this.viewmodel = viewmodel;
                this.canvas = canvas;
                this.model = model;
                ...
                this.dimensionRenderer = new Floorplanner.DimensionRenderer(this.context, this.viewmodel, {
                    offset: 60,
                    lineColor: '#333333',
                    textColor: '#000000'
                });
            ...
            FloorplannerView.prototype.getItems = function () {
                try {
            ...
            FloorplannerView.prototype.drawItem = function (item) {
                try {
...
(function (BP3D) {
    ...
    (function (Floorplanner_1) {
        ...
        var Floorplanner = (function () {
            /** */
            function Floorplanner(canvas, floorplan, model) {
                this.floorplan = floorplan;
                this.model = model;
                /** */
                this.mode = 0;
                ...
                this.view = new Floorplanner_1.FloorplannerView(this.floorplan, this, canvas, this.model);
                var cmPerFoot = 30.48;
            ...
            Floorplanner.prototype.getItemAt = function (x, y) {
                if (!this.model || !this.model.scene)
                    return null;
                try {
            ...
            Floorplanner.prototype.getPixelsPerCm = function () {
    ...
    (function (Three) {
        Three.Floor = function (scene, room) {
            ...
            function buildFloor() {
                ...
                var floorTexture = THREE.ImageUtils.loadTexture(textureSettings.url, null, function () {
...
(function (BP3D) {
    ...
    (function (Three) {
        ...
        Three.setEdgeAutoVisibility = function (enabled) {
            disableAutoVisibility = !enabled;
        };
        Three.getEdgeAutoVisibility = function () {
...
(function (BP3D) {
    ...
    var Blueprint3d = (function () {
        ...
        function Blueprint3d(options) {
            ...
            if (!options.widget) {
                this.floorplanner = new BP3D.Floorplanner.Floorplanner(options.floorplannerElement, this.model.floorplan, this.model);
            }
            else {
...
(function (BP3D) {
    ...
    (function (Core) {
        var GeometryUtils = (function () {
             ...
             */
            GeometryUtils.isCounterClockwise = function (points) {
             ...
             */
            GeometryUtils.ensureCounterClockwise = function (points) {
             ...
             */
            GeometryUtils.calculateCentroid = function (points) {
                var sumX = 0;
                var sumY = 0;
                points.forEach(function (p) {
                    sumX += p.x;
             ...
             */
            GeometryUtils.distance = function (p1, p2) {
                var dx = p2.x - p1.x;
                var dy = p2.y - p1.y;
             ...
             */
            GeometryUtils.direction = function (p1, p2) {
                var dx = p2.x - p1.x;
                var dy = p2.y - p1.y;
                var length = Math.sqrt(dx * dx + dy * dy);
                return {
             ...
             */
            GeometryUtils.perpendicular = function (v) {
                return { x: -v.y, y: v.x };
            };
            /**
             * Calculate dot product of two vectors
             */
            GeometryUtils.dotProduct = function (v1, v2) {
                return v1.x * v2.x + v1.y * v2.y;
            };
            /**
             * Calculate cross product magnitude (2D)
             */
            GeometryUtils.crossProduct = function (v1, v2) {
                return v1.x * v2.y - v1.y * v2.x;
            };
            /**
             * Scale a vector by a scalar value
             */
            GeometryUtils.scale = function (v, scalar) {
                return { x: v.x * scalar, y: v.y * scalar };
            };
            /**
             * Add two vectors
             */
            GeometryUtils.add = function (v1, v2) {
                return { x: v1.x + v2.x, y: v1.y + v2.y };
            };
            /**
             * Subtract two vectors (v1 - v2)
             */
            GeometryUtils.subtract = function (v1, v2) {
                return { x: v1.x - v2.x, y: v1.y - v2.y };
            };
            /**
             * Normalize a vector to unit length
             */
            GeometryUtils.normalize = function (v) {
             ...
             */
            GeometryUtils.midpoint = function (p1, p2) {
                return {
                    x: (p1.x + p2.x) / 2,
                    y: (p1.y + p2.y) / 2
                };
             ...
             */
            GeometryUtils.isPointInPolygon = function (point, polygon) {
                var inside = false;
             ...
             */
            GeometryUtils.angleBetween = function (v1, v2) {
                var dot = this.dotProduct(v1, v2);
             ...
             */
            GeometryUtils.rotate = function (point, angle, origin) {
             ...
             */
            GeometryUtils.boundingBox = function (points) {
                if (points.length === 0) {
        ...
        Core.GeometryUtils = GeometryUtils;
    })(Core = BP3D.Core || (BP3D.Core = {}));
})(BP3D || (BP3D = {}));
var BP3D;
(function (BP3D) {
...
(function (BP3D) {
    ...
    (function (Core) {
        var SVGPathParser = (function () {
             ...
             */
            SVGPathParser.parse = function (pathData) {
                var points = [];
                // Match M and L commands with their coordinates
                var commandRegex = /([ML])\s*([\d.]+)\s*,\s*([\d.]+)/gi;
                var match;
                while ((match = commandRegex.exec(pathData)) !== null) {
                    ...
                    if (!isNaN(x) && !isNaN(y)) {
                        points.push({ x: x, y: y, command: command });
                    }
                }
                return points;
            };
             ...
             */
            SVGPathParser.parseCoordinates = function (pathData) {
                var pathPoints = this.parse(pathData);
                return pathPoints.map(function (p) { return ({ x: p.x, y: p.y }); });
            };
            /**
             * Transform SVG coordinates to Blueprint3D coordinate system
             */
            SVGPathParser.transform = function (points, options) {
                if (options === void 0) { options = {}; }
                var _a = options.scale, scale = _a === void 0 ? 1 : _a, _b = options.offsetX, offsetX = _b === void 0 ? 0 : _b, _c = options.offsetY, offsetY = _c === void 0 ? 0 : _c, _d = options.centerX, centerX = _d === void 0 ? 0 : _d, _e = options.centerY, centerY = _e === void 0 ? 0 : _e, _f = options.ensureCounterClockwise, ensureCounterClockwise = _f === void 0 ? true : _f;
                // Apply transformation
                var transformed = points.map(function (p) { return ({
                    x: (p.x - centerX) * scale + offsetX,
                    y: (p.y - centerY) * scale + offsetY
                }); });
                // Ensure counter-clockwise if requested
                if (ensureCounterClockwise) {
                    transformed = Core.GeometryUtils.ensureCounterClockwise(transformed);
                }
                return transformed;
            };
            /**
             ...
             */
            SVGPathParser.parseAndTransform = function (pathData, options) {
                if (options === void 0) { options = {}; }
                var points = this.parseCoordinates(pathData);
             ...
             */
            SVGPathParser.svgToBlueprint3D = function (points, svgViewBoxSize, roomScale) {
                if (svgViewBoxSize === void 0) { svgViewBoxSize = 60; }
                if (roomScale === void 0) { roomScale = 15; }
                var center = svgViewBoxSize / 2;
             ...
             */
            SVGPathParser.validate = function (pathData) {
                var errors = [];
                var points = this.parseCoordinates(pathData);
             ...
             */
            SVGPathParser.pointsToPath = function (points) {
                if (points.length === 0)
                    return '';
                var pathCommands = [];
             ...
             */
            SVGPathParser.getPathBounds = function (pathData) {
                var points = this.parseCoordinates(pathData);
                var bbox = Core.GeometryUtils.boundingBox(points);
                return {
                    min: bbox.min,
                    max: bbox.max,
             ...
             */
            SVGPathParser.normalizePath = function (pathData, targetWidth, targetHeight, maintainAspectRatio) {
                ...
                return points.map(function (p) { return ({
        ...
        Core.SVGPathParser = SVGPathParser;
    })(Core = BP3D.Core || (BP3D.Core = {}));
})(BP3D || (BP3D = {}));
var BP3D;
(function (BP3D) {
